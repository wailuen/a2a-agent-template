...
9d. **Write credential inventory ADR.**
    After all seeding steps (9, 9b, 9c) complete, call `GET /admin/api/credentials`
    and write (or overwrite) `workspace/adr/ADR-001-<env>-credentials.md`:
    ...
    updated: <ISO date>
    ---
    # Credential Inventory — <env>

    Auto-generated by /provision. Records which namespaces and fields are seeded.
    Never contains values. Regenerate by re-running /provision.

    ## Admin
    - API key: minted <date> — stored in ADR-000-<env>-credentials.md

    ## Credential store
    - Backend: SQLite (local) | Azure Table Storage
    - Table: <prefix>credentials | n/a
    - Auth: connection string | DefaultAzureCredential | n/a
    ...
    | __bedrock__ | aws_access_key_id | ✓ / ✗ |

    ## Infrastructure (.env routing vars — non-secret)
    | Var | Value |
    |----|-------|
    | AZURE_STORAGE_ACCOUNT_NAME | <value or n/a> |
    | AZURE_STORAGE_TABLE_PREFIX | <value or n/a> |
    | OPENAI_MODEL / BEDROCK_MODEL_ARN / AZURE_OPENAI_ENDPOINT | <value> |
    ```

    Ensure `workspace/adr/ADR-001-*-credentials.md` is in `.gitignore`
    (add if missing — same block as ADR-000).
    **Never write a credential value into this file — only namespace, field name, and set status.**
...
ADR-001:         written to workspace/adr/ADR-001-<env>-credentials.md
